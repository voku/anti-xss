...
final class AntiXSS
{
    ...
    {
        /** @var array{chunk: array<string, string>, split: array<string, array<int, string>>}|null $WORDS_CACHE */
     ...
     * @return string
     */
    private function _decode_entity($match): string
    {
        // init
        $str = $match[0];
                ...
                foreach ($matchesTmp as $matches) {
                    if (isset($matches['attr'])) {
                        ...
                        if ($tmpAntiXss->isXssFound() === true) {
                            ...
                            $urlPartClean = \str_replace(['&lt;', '&gt;'],
                                                         [self::VOKU_ANTI_XSS_LT, self::VOKU_ANTI_XSS_GT],
                                                         $urlPartClean);
                            $urlPartClean = UTF8::rawurldecode($urlPartClean);
                            $urlPartClean = \str_replace([self::VOKU_ANTI_XSS_LT, self::VOKU_ANTI_XSS_GT],
                                                         ['&lt;', '&gt;'], $urlPartClean);
     ...
     * @return string
     */
    private function _decode_string($str): string
    {
        // init
    ...
    private function _do($str)
    {
        ...
        do {
            ...
            $loop_count++;
        } while ($str_backup_loop !== $str && $loop_count < $max_loops);
     ...
     * @return  string
     */
    private function _do_never_allowed_afterwards($str): string
    {
            ...
            foreach ($this->_get_never_allowed_on_events_afterwards_chunks() as $eventNameBeginning => $events) {
                ...
                foreach ($events as $event) {
                    ...
                    do {
                        ...
                        $count += $temp_count;
                        $inner_loop_count++;
                    } while ($count && $inner_loop_count < $max_inner_loops);
    ...
    private function _js_removal_callback(array $match, string $search): string
    {
        ...
        if (\stripos($match[1], $search . '=') !== false) {
            ...
            if (\preg_match($pattern, $match[1], $matchInner)) {
                ...
                if (
                    !str_contains($matchInner[0], 'script')
                    &&
                    !str_contains(\str_replace(['http://', 'https://'], '', $matchInner[0]), ':')
                    &&
                    (
                        \filter_var($matchInner['link'], \FILTER_VALIDATE_URL) !== false
                        ||
                        \filter_var('https://localhost.localdomain/' . $matchInner['link'],
            ...
            if (!$foundSomethingBad) {
                ...
                $pattern    = '#' . $search . '=.*(?:' . $patternTmp . \implode('|',
                                                                                $this->_never_allowed_js_callback_regex) . ')#ius';
                $matchInner = [];
                if (\preg_match($pattern, $match[1], $matchInner)) {
                    $replacer = (string)\preg_replace(
                        $pattern,
        ...
        if (
            ...
            &&
            str_ends_with($match[1], ' /')
            &&
            !str_ends_with($replacer, ' /')
        ) {
            $replacer .= ' /';
        } elseif (
            str_ends_with($match[0], '/>')
        ...
        do {
            ...
            if (\stripos($str, '<img') !== false) {
                $strTmp = \preg_replace_callback(
                    ...
                    function ($matches) {
                        if (
                            str_contains($matches[1], 'base64')
                            &&
                            \preg_match("/([\"'])?data\s*:\s*(?:image\s*\/.*)[^\1]*base64[^\1]*,[^\1]*\1?/iUus",
                                        $matches[1])
                        ) {
                            return $matches[0];
                        }
            ...
            $loop_count++;
        } while ($original !== $str && $loop_count < $max_loops);
     ...
     * @return string
     */
    private function _repack_utf7_callback_back(string $str): string
    {
    ...
    private function _close_html_callback(array $matches): string
    {
        if (empty($matches['closeTag'])) {
            ...
            if (\preg_match('/^[ .,\d=%€$₢₣£₤₶ℳ₥₦₧₨රුரூ௹रू₹૱₩₪₸₫֏₭₺₼₮₯₰₷₱﷼₲₾₳₴₽₵₡¢¥円৳元៛₠¤฿؋]*$|^[ .,\d=%€$₢₣£₤₶ℳ₥₦₧₨රුரூ௹रू₹૱₩₪₸₫֏₭₺₼₮₯₰₷₱﷼₲₾₳₴₽₵₡¢¥円৳元៛₠¤฿؋]+\p{L}*\s*$/u',
                            $matches[1])) {
    ...
    public function xss_clean(array|string $str): array|string
    {
        ...
        do {
            ...
            $str     = $this->_do($str);
            $xss_loop_count++;
        } while ($old_str !== $str && $xss_loop_count < $max_xss_loops);
